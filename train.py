...
def train_net(sym, roidb, args):
    ...
    #print(feat_sym.list_outputs())#['rpn_cls_score_output']
...
def parse_args():
    ...
    parser.add_argument('--img-short-side', type=int, default=300)
    parser.add_argument('--img-long-side', type=int, default=500)
    ...
    parser.add_argument('--rpn-pre-nms-topk', type=int, default=12)
    parser.add_argument('--rpn-post-nms-topk', type=int, default=2)
    ...
    parser.add_argument('--rpn-batch-rois', type=int, default=8)#配置低，调低
    ...
    parser.add_argument('--rcnn-batch-rois', type=int, default=4)#配置低，调低
...
def get_voc(args):
    ...
    for iset in isets:
        ...
        #这里取决于你的数据集存放的路径了，我的放这个根目录的上级目录
        imdb = PascalVOC(iset, '../data', '../data/VOCdevkit')
        imdb.filter_roidb()
...
def main():
    ...
    #print(type(roidb),len(roidb))#<class 'list'> 10022
