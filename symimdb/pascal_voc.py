...
class PascalVOC(IMDB):
    classes = ['__background__',  # 背景索引0，总计21个类别
    ...
    def voc_eval(class_anno, npos, image_ids, bbox, confidence, ovthresh=0.5, use_07_metric=False):
        # 置信度排序，注意-confidence，这样就变成从大到小的排序了
        if bbox.shape[0] > 0:
        ...
        # 向下检测并标记真阳性tp和假阳性fp(False Positives)
        nd = len(image_ids)
        tp = np.zeros(nd)
        fp = np.zeros(nd)
        for d in range(nd):
            ...
            if bbgt.size > 0:
                # 交并比IoU
                # 交集（两者左上角取最大值，右下角取最小值），求出宽高，然后面积就是相交的部分
